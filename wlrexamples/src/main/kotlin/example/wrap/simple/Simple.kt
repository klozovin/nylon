...
fun outputFrameNotify(listener: MemorySegment, data: MemorySegment) {
    ...
    val pass = backend_h.wlr_output_begin_render_pass(Output.output, statePtr, MemorySegment.NULL, MemorySegment.NULL)
...
fun outputRemoveNotify(listener: MemorySegment, data: MemorySegment) {
    ...
    // wlr_log(WLR_DEBUG, "examples.direct.Output removed");
    Log.logDebug("examples.direct.Output removed")
...
fun newOutputNotify(listenerPtr: MemorySegment, outputPtr: MemorySegment) {
    ...
    // examples.direct.wl_signal_add(&output->events.frame, &sample_output->frame);
    ...
    // examples.direct.wl_signal_add(&output->events.destroy, &sample_output->destroy);
...
fun newInputNotify(listenerPtr: MemorySegment, inputDevicePtr: MemorySegment) {
    ...
    if (wlr_input_device.type(inputDevicePtr) == wlr_input_device_h.WLR_INPUT_DEVICE_KEYBOARD()) {
        ...
        // examples.direct.wl_signal_add(&device->events.destroy, &keyboard->destroy);
        ...
        // examples.direct.wl_signal_add(&keyboard->wlr_keyboard->events.key, &keyboard->key);
...
fun main() {
    ...
    // examples.direct.wl_signal_add(&backend->events.new_output, &state.new_output);
    ...
    // examples.direct.wl_signal_add(&backend->events.new_input, &state.new_input);
 ...
 *
 * ```
 * static inline void examples.direct.wl_signal_add(struct wl_signal *signal, struct wl_listener *listener) {
