...
import static jextract.xkbcommon.xkb.xkb_state_key_get_one_sym;
import static jextract.xkbcommon.xkb.xkb_state_key_get_syms;
...
public class XkbState {
    ...
    ///
    /// This function is similar to {@link #keyGetSyms(int)}, but intended for users which cannot or do not
    /// want to handle the case where multiple keysyms are returned (in which case this function is
    /// preferred).
    ///
    /// This function performs Capitalization [keysym
    /// transformation](https://xkbcommon.org/doc/current/group__keysyms.html).
    ///
    ...
    public int[] keyGetSyms(int keycode) {
        try (var arena = Arena.ofConfined()) {
            var keySymsPtr = arena.allocate(xkb.C_POINTER);
            ...
            for (int i = 0; i < numKeySyms; i++) {
                keySyms[i] = keySymsPtr.get(xkb.C_POINTER, i).get(xkb.xkb_keysym_t, 0);
            }
            return keySyms;
        }
    }
